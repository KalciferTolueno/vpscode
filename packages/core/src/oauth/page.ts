  ...
  return renderDocument({
    ...
    body: renderCard({
      ...
      headline: "Authorization successful",
       message: provider ? `VPS Code is now connected to ${escapeHtml(provider)}.` : "VPS Code is now authorized.",
...
export function error(detail: string, options?: CallbackPageOptions) {
  ...
  return renderDocument({
    ...
    body: renderCard({
      ...
      message: provider
         ? `VPS Code couldn't finish connecting to ${escapeHtml(provider)}.`
         : "VPS Code couldn't complete authorization.",
      detail,
       footnote: "Close this window and try again from VPS Code.",
    }),
  })
}
...
<html lang="en">
  <head>
    ...
    <title>${escapeHtml(input.title)} · VPS Code</title>
    <style>${STYLES}</style>
  </head>
  <body>
...
(function(){
  ...
  function fail(text){card.dataset.status="error";headline.textContent="Authorization failed";message.textContent=PROVIDER?("VPS Code couldn't finish connecting to "+PROVIDER+"."):"VPS Code couldn't complete authorization.";if(text){detail.textContent=text;detail.hidden=false}footnote.textContent="Close this window and try again from VPS Code."}
  function ok(){card.dataset.status="success";headline.textContent="Authorization successful";message.textContent=PROVIDER?("VPS Code is now connected to "+PROVIDER+"."):"VPS Code is now authorized.";detail.hidden=true;footnote.textContent="You can close this window.";setTimeout(function(){try{window.close()}catch(e){}},2500)}
  try{
...
// VPS Code wordmark — same treatment as packages/ui/src/components/logo.tsx (Logo).
const WORDMARK = `<svg class="wordmark" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 234 42" fill="none" aria-label="VPS Code" role="img">
        <path d="M18 30H6V18H18V30Z" fill="var(--oc-icon-weak)" />
        <path d="M18 12H6V30H18V12ZM24 36H0V6H24V36Z" fill="var(--oc-icon-base)" />
        <text x="36" y="31" fill="var(--oc-icon-strong)" font-family="var(--oc-font-mono)" font-size="28" font-weight="700" letter-spacing="0.5">VPS Code</text>
      </svg>`
